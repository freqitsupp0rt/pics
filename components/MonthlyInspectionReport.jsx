  ...
  const applyImageAdjustments = (imageDataUrl, brightness, sharpness) => {//for brightness and sharpness adjustments in lightbox
    ...
    doc.text("Notes: Photos should have Geotagging (coordinates, date and time stamp)", 15, footerY + 3);
    ...
    const innerPadding = 4 ;  // table padding
    const gapX = 3;         // x-axis gap
    const gapY = 8;          // y-axis gap
    ...
    const verticalOffset = Math.max(0, (totalHeight - gridTotalHeight) / 2);
  ...
  doc.text(dateText, pageWidth - 15, doc.lastAutoTable.finalY + 10, { align: 'right' });
        ...
        autoTable(doc, {
          startY: doc.lastAutoTable.finalY + 15,
            margin: { left: 7.5, right: 7.5 }, // reduce margins to make table wider
          head: [[
            'Item\nNo.',
            'Location Code',
            'Location Name',
    ...
    1: { halign: 'center', cellWidth: 40 }, // Location code width
    2: { halign: 'left', cellWidth: 59 }, // Location name width table
    5: { cellWidth: 22 }, // Contracted Bandwidth
    6: { cellWidth: 19 } , // Remarks
    3: { cellWidth: 22 }, // Downlink Bandwidth
    4: { cellWidth: 22 }, // Uplink Bandwidth
  }
        });
        ...
        const sectionHeight = 200;//table height
        doc.setDrawColor(0);
        doc.setLineWidth(0.1);
        doc.rect(15, currentY, pageWidth - 30, sectionHeight);

        if (comboxImage) {
          addImageToPage(comboxImage, 8 + 19, currentY + 13, pageWidth - 54, sectionHeight - 26);
        } else {
          doc.setFont('Palatino', 'italic');
    ...
    doc.setFontSize(11);
    doc.setFont('Palatino', 'bold');
    const titleY = 40; // moved up
    doc.text("ATTACHMENT 1: EQUIPMENT PHOTOS (Access Points)", pageWidth / 2, titleY, { align: 'center' });
    const gridStartY = titleY + 5;
    const gridHeight = pageHeight - gridStartY - 52;
    doc.rect(8, gridStartY, pageWidth - 16, gridHeight);
    draw2x2Grid(additionalImages, gridStartY, gridHeight, 8);
  }
  ...
  doc.setFontSize(11);
  doc.setFont('Palatino', 'bold');
  const bwTitleY = 40; // moved up
  doc.text("ATTACHMENT 2: BANDWIDTH TEST RESULTS", pageWidth / 2, bwTitleY, { align: 'center' });
  const bwGridStartY = bwTitleY + 5;
  const bwGridHeight = pageHeight - bwGridStartY - 53 ; // taller
  doc.rect(8, bwGridStartY, pageWidth - 16, bwGridHeight); // wider

  if (speedtestImages.length > 0) {
    draw2x2Grid(speedtestImages, bwGridStartY, bwGridHeight, 8); // match margin
  } else {
    doc.setFont('Palatino', 'italic');
    doc.setFontSize(10);
  ...
  doc.setFontSize(11);
  doc.setFont('Palatino', 'bold');
  const siteTitleY = 40; // moved up
  doc.text("ATTACHMENT 3: SITE BENEFICIARY", pageWidth / 2, siteTitleY, { align: 'center' });

  const siteSectionY = siteTitleY + 5;
  const siteHeight = pageHeight - siteSectionY - 53; // taller
  doc.rect(8, siteSectionY, pageWidth - 16, siteHeight); // wider

  if (siteInspectionImage) {
  addImageToPage(siteInspectionImage, 8 + 19, siteSectionY + 13, pageWidth - 54, siteHeight - 26);
  } else {
    doc.setFont('Palatino', 'italic');
    doc.setFontSize(10);
